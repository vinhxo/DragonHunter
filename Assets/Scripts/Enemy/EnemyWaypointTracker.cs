...
public class EnemyWaypointTracker : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        bossStateChecker = GameObject.FindGameObjectWithTag("Boss").GetComponent<BossState>();
    }
    void Start()
    {
        agent.avoidancePriority = Random.Range(4, 15);
    ...
    void MoveAndAttack()
    {
        ...
        if (distance > walkDistance)
        {
            if (enemyHealth.currentHealth < enemyHealth.maxHealth || 
               bossStateChecker.enemyHealth.currentHealth < bossStateChecker.enemyHealth.maxHealth)
            {
